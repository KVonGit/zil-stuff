	...
	.WORD 0
	.WORD 0
	.INSERT ".\followers-test_freq"
	.INSERT ".\followers-test_data"

	.FUNCT GO
START::
	PRINTI "Follower Demo Initialized."
	CRLF
	SET 'HERE,ROOM-1
	MOVE PLAYER,HERE
	ICALL FOLLOW,FOLLOWER-NPC,PLAYER
	ICALL1 V-LOOK
	ICALL1 MAIN-LOOP
	QUIT
...
?L1:	CRLF
	...
	.FUNCT FIND-DIRECTION-FROM,FROM,L,P,PT
	SET 'P,64
?L1:	DLESS? 'P,P?DOWN /TRUE
	GETPT HERE,P >PT
	ZERO? PT /?L1
	PTSIZE PT >L
	EQUAL? L,UEXIT,DEXIT,CEXIT \?L1
	GET PT,EXIT-RM >STACK
	EQUAL? STACK,FROM \?L1
	RETURN P

	.FUNCT FIND-PATH,START,DEST,QUEUE?1,VISITED,PARENT,IDX,CURRENT,NEXT,EXIT,RM,D,PT
	SET 'QUEUE?1,T?103
	SET 'VISITED,T?104
	SET 'PARENT,T?105
	PUTB VISITED,0,START
	PUTB QUEUE?1,0,START
	SET 'IDX,1
?L1:	ZERO? IDX /FALSE
	SUB IDX,1 >STACK
	GETB QUEUE?1,STACK >CURRENT
	EQUAL? CURRENT,DEST /TRUE
	SET 'D,64
?L9:	DLESS? 'D,P?DOWN /?L1
	GETPT CURRENT,D >PT
	ZERO? PT /?L9
	GET PT,REXIT >RM
	IN? RM,VISITED /?L9
	...
	.FUNCT BUILD-PATH,START,DEST,PARENT,PATH,IDX,CUR,PREV,DIR
	PTSIZE PARENT >STACK
	SUB STACK,1 >IDX
	SET 'CUR,DEST
	SET 'FOLLOWER-IDX,0
?L1:	EQUAL? CUR,START /TRUE
	GETB PARENT,IDX >PREV
	CALL2 FIND-DIRECTION-FROM,PREV >DIR
	...
	.FUNCT FOLLOW,FOLLOWER,TARGET
	SET 'FOLLOWER-IDX,0
	PUTB FOLLOWER-TABLE,0,FOLLOWER
	SET 'FOLLOWER-TABLE,1
	LOC TARGET >STACK
	LOC FOLLOWER >STACK
	CALL FIND-PATH,STACK,STACK >STACK
	ZERO? STACK /FALSE
	LOC FOLLOWER >STACK
	LOC TARGET >STACK
	LOC FOLLOWER >STACK
	CALL BUILD-PATH,STACK,STACK,STACK >STACK
	RSTACK

	.FUNCT FOLLOW-TICK,FOLLOWER,DIR,DEST
	LESS? FOLLOWER-IDX,0 /FALSE
	DEC 'FOLLOWER-IDX
	SUB FOLLOWER-IDX,1 >STACK
	GETB FOLLOWER-PATH,STACK >DIR
	LOC FOLLOWER >STACK
	GET STACK,DIR >DEST
	MOVE FOLLOWER,DEST
	RTRUE

	.FUNCT FOLLOWER-F,ARG
	EQUAL? ARG,M-ENTER \FALSE
	CALL2 FOLLOW-TICK,FOLLOWER-NPC >STACK
	RSTACK

	.INSERT ".\followers-test_str"
	.END
